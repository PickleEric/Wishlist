...
def place_list(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid(): # validation against DB constraints
            ...
            return redirect('place_list') # reolads home page
...
def place_details(request, place_pk):
    ...
    # Does the place belong to current user
    if place.user != request.user:
        return HttpResponseForbidden()

    # is it a GET request (show data + form), or a POST request(update Place object?)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request, form.errors) # temp fix later
    ...
    # if place is visisted, show form; if place is not visisted, no form.
        if place.visited:
